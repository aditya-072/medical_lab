...
function Delete({ usersArray }) {
    // console.log(patientsArray);
    // const [name, setName] = useState(usersArray[0].name);
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await axios.delete(`/api/users/${usersArray[0].id}`);
...
export async function getServerSideProps(context) {
    ...
    const usersArray = response.data.data.usersArray;
    // console.log(patientsArray);

    // Return the data as props
    return { props: { usersArray } };
}
